with users_with_order_info as (
 ...
 ), agregate_for_monetary_recency as (
	select 
	   id
	  ,order_id
	  ,case
		  when stutus_discr = 'Closed' then date_part('day', now() - order_ts)
		  else 9999999
	   end as days_from_closed_order
   from users_with_order_info
 ), agregate_for_monetary_recency_2 as (
 	select 	
	   id
	  ,min(days_from_closed_order) as days_from_last_closed_order
   from agregate_for_monetary_recency
   group by id
 ), agregate_for_monetary_recency_3 as (
	select 	
	   *
	   ,row_number() over (order by days_from_last_closed_order desc) as rn
   from agregate_for_monetary_recency_2
 )
...
insert into analysis.tmp_rfm_recency
 ...
 from agregate_for_monetary_recency_3
